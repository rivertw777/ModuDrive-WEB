...
export function AddMemberForm({
  ...
  const doShare = async (targetEmails: string[]) => {
    ...
    const failures = targetEmails
      ...
      .filter(
        (f): f is { email: string; result: PromiseRejectedResult } =>
    ...
    <>
        ...
        <div className="flex items-start gap-2">
          <div className="flex min-h-[2.875rem] min-w-0 flex-1 flex-wrap content-center items-center gap-1.5 rounded-lg border border-slate-300 p-2 focus-within:border-brand-500 dark:border-slate-600">
            {emails.map((email) => (
              <span
                key={email}
                className="inline-flex max-w-full items-center gap-1 rounded-full bg-slate-100 px-2 py-1 text-sm text-slate-700 dark:bg-slate-700 dark:text-slate-200"
              >
                ...
                <button
                  ...
                  className="shrink-0 text-slate-400 hover:text-slate-700 dark:hover:text-slate-100"
                >
                  <XIcon size={12} />
                </button>
              </span>
      ...
      {guestWarning !== null && (
        <ConfirmDialog
          ...
          message={`${guestWarning.join(', ')}님에게 초대장을 전송하려고 합니다. 이 이메일 계정과 연결된 ModuDrive 계정이 없기 때문에, 이 초대 링크를 전달받은 사람은 누구나 로그인 없이 액세스할 수 있습니다.`}
