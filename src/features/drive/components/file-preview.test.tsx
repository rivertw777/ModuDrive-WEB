...
describe('FilePreview', () => {
  it('renders nothing for a non-previewable file', () => {
    const { container } = render(
      <FilePreview fileName="report.pdf" source={{ type: 'auth', fileId: 'f-1' }} />,
    )
  ...
  it('renders an image via the authenticated source', async () => {
    ...
    render(<FilePreview fileName="photo.png" source={{ type: 'auth', fileId: 'f-1' }} />)
  ...
  it('renders a video via the public link source', async () => {
    ...
    const { container } = render(
      <FilePreview fileName="clip.mp4" source={{ type: 'public', token: 'tok-1' }} />,
    )
  ...
  it('renders audio controls', async () => {
    ...
    const { container } = render(
      <FilePreview fileName="song.mp3" source={{ type: 'auth', fileId: 'f-1' }} />,
    )
  ...
  it('renders fetched text content for a .txt file', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ text: () => Promise.resolve('hello world') }))

    render(<FilePreview fileName="notes.txt" source={{ type: 'auth', fileId: 'f-1' }} />)
