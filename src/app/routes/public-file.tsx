 ...
 * with their full context — not the stripped anonymous page. Until we know, or if they turn
 * out not to have access (a guest whose claim hasn't landed yet, or someone signed in as a
 * different account), fall back to the anonymous `key` view. A folder also stays on the
 * anonymous view — `/files/:fileId` is a single-file detail panel with no way to browse in.
 */
...
function AuthenticatedPublicRedirect({
  ...
  const { data, isLoading, isError } = useFile(fileId)
  if (isLoading) return <LoadingState />
  if (isError || data?.directory) return <PublicFileView fileId={fileId} shareKey={shareKey} />
